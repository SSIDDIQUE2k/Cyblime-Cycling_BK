    ...
    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <h4 className="font-semibold text-[#2A2A2A] mb-1">{goal.title}</h4>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  variant="outline" className="bg-orange-400 text-white px-4 py-2 text-sm font-medium rounded-md inline-flex items-center justify-center gap-2 whitespace-nowrap transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 border shadow-sm hover:text-accent-foreground h-9 border-white/30 hover:bg-white/10">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <Tabs defaultValue="events" className="space-y-8">
            ...
            <TabsContent value="events" className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">My Created Events ({myCreatedEvents.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <p className="text-center text-[#555555] py-8">No events created yet</p>
                  }
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">My Tickets ({userTickets.filter((t) => t.payment_status === 'completed').length})</CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    <div key={ticket.id} className="p-4 border-2 border-[#ff6b35] rounded-xl bg-gradient-to-br from-[#ff6b35]/5 to-transparent">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              ...
                              <Badge className="bg-[#A4FF4F] text-[#2A2A2A] border-0 text-xs">
                                {ticket.ticket_type}
                              </Badge>
                            </div>
                  ...
                  <p className="text-center text-[#555555] py-8">No tickets purchased yet</p>
                  }
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">Upcoming Events ({upcomingEvents.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="space-y-3">
                    ...
                    <div key={reg.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                          <div className="flex-1">
                            <h4 className="font-semibold text-[#2A2A2A] mb-1">{reg.event_name}</h4>
                            <div className="flex items-center gap-4 text-sm text-[#555555]">
                              <div className="flex items-center gap-1">
                                <Calendar className="w-3 h-3" />
                  ...
                  <p className="text-center text-[#555555] py-8">No upcoming events registered</p>
                  }
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">Past Events ({pastEvents.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  {pastEvents.length > 0 ?
                  <div className="space-y-3">
                      {pastEvents.slice(0, 5).map((reg) =>
                    <div key={reg.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-xl">
                          <div className="flex-1">
                            <h4 className="font-semibold text-[#2A2A2A] mb-1">{reg.event_name}</h4>
                            <div className="flex items-center gap-4 text-sm text-[#555555]">
                              <div className="flex items-center gap-1">
                          ...
                          {reg.status === 'attended' &&
                      <Badge className="bg-[#A4FF4F] text-[#2A2A2A] border-0 font-bold">
                              <CheckCircle2 className="w-3 h-3 mr-1" />
                              Attended
                            </Badge>
                      }
                  ...
                  <p className="text-center text-[#555555] py-8">No past events</p>
                  }
                </CardContent>
              </Card>
            </TabsContent>
            ...
            <TabsContent value="goals">
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <CardTitle className="text-[#2A2A2A]">Cycling Goals</CardTitle>
                  <Button
                    onClick={() => {
                      setEditingGoal(null);
                      setGoalDialogOpen(true);
            ...
            <TabsContent value="achievements" className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">Badges ({userBadges.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  {userBadges.length > 0 ?
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">Points Breakdown</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 bg-gradient-to-br from-[#6BCBFF]/10 to-[#6BCBFF]/5 rounded-xl">
                      <div className="text-3xl font-bold text-[#2A2A2A] mb-1">{userPoints?.weekly_points || 0}</div>
                      <div className="text-sm text-[#555555]">This Week</div>
                    </div>
                    <div className="p-4 bg-gradient-to-br from-[#A4FF4F]/10 to-[#A4FF4F]/5 rounded-xl">
                      <div className="text-3xl font-bold text-[#2A2A2A] mb-1">{userPoints?.monthly_points || 0}</div>
                      <div className="text-sm text-[#555555]">This Month</div>
                    </div>
                  </div>
                  
                  <div className="mt-6">
                    <h4 className="font-semibold text-[#2A2A2A] mb-3">How to Earn Points</h4>
            ...
            <TabsContent value="routes">
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">Saved Routes ({savedRoutes.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  {savedRoutes.length > 0 ?
                  <div className="grid md:grid-cols-2 gap-4">
                      {savedRoutes.map((route) =>
                    <div key={route.id} className="p-4 border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                          <h4 className="font-semibold text-[#2A2A2A] mb-2">{route.name}</h4>
            ...
            <TabsContent value="notes">
              <Card>
                <CardHeader>
                  <CardTitle className="text-[#2A2A2A]">Private Notes</CardTitle>
                </CardHeader>
                <CardContent>
                  <Textarea
