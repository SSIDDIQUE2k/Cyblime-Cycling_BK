...
export default function RouteDetails() {
  ...
  const renderDescriptionWithImages = () => {
    ...
    sections.forEach((section, index) => {
      elements.push(
        <p key={`p-${index}`} className="text-[#555555] text-lg leading-relaxed mb-6">
          {section}
        </p>
      );
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center">
        <div className="text-[#555555]">Loading route...</div>
      </div>
    );
  }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
          ...
          >
            <div className="bg-gradient-to-br from-[#6BCBFF]/10 to-[#6BCBFF]/5 rounded-2xl p-6">
              ...
              <div className="text-3xl font-bold text-[#2A2A2A] mb-1">{route.distance}km</div>
              <div className="text-sm text-[#555555]">Distance</div>
            </div>
            ...
            <div className="bg-gradient-to-br from-purple-400/10 to-purple-400/5 rounded-2xl p-6">
              ...
              <div className="text-lg font-bold text-[#2A2A2A] mb-1">{route.surface_type}</div>
              <div className="text-sm text-[#555555]">Surface Type</div>
            </div>
          </motion.div>
          ...
          >
            <h2 className="text-3xl font-bold text-[#2A2A2A] mb-6">About This Route</h2>
          ...
          <motion.div
            ...
            className="grid md:grid-cols-2 gap-6 mb-12 pb-12 border-b border-gray-200"
          >
            <div>
              <h3 className="text-xl font-bold text-[#2A2A2A] mb-4">Route Details</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                  <span className="text-[#555555]">Start Location</span>
                  <span className="font-semibold text-[#2A2A2A]">{route.start_location}</span>
                </div>
                {route.end_location && (
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                    <span className="text-[#555555]">End Location</span>
                    <span className="font-semibold text-[#2A2A2A]">{route.end_location}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {route.highlights && route.highlights.length > 0 && (
              <div>
                <h3 className="text-xl font-bold text-[#2A2A2A] mb-4">Highlights</h3>
          ...
          >
            ...
            {user ? (
              ...
              <div className="mb-8 p-6 bg-gray-50 rounded-2xl text-center">
                <p className="text-[#555555]">Please sign in to leave a review</p>
              </div>
            )}
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-2">
                          <div>
                            <div className="font-semibold text-[#2A2A2A]">{comment.created_by}</div>
                        ...
                        <p className="text-[#555555] leading-relaxed">{comment.comment}</p>
                      </div>
                    </div>
                  </motion.div>
                ))}
